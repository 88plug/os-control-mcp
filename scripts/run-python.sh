...
find_python() {
  ...
  for c in "${EIGHTYEIGHT_PYTHON:-}" "${PLUGIN_PYTHON:-}" "${OS_CONTROL_PYTHON:-}"; do
    _try "$c" && return 0
  done
...
PY="$(find_python)" || {
  echo "os-control-mcp: no Python >=${MIN_MAJOR}.${MIN_MINOR} found." >&2
  echo "  Set EIGHTYEIGHT_PYTHON=/path/to/python3 (or OS_CONTROL_PYTHON), or install Python 3." >&2
